...
import {
  IThreadControllerDeleteThreadOutputError as IControllerError,
  IThreadControllerDeleteThreadOutputSuccess as IControllerSuccess,
...
/**
 * @name					threadRouterDeleteThread
 *
 * @description		The 'deleteThread' router for 'thread', whose expected
 *                inputs have been defined within the following schema:
 *                'threadValidationSchemaDeleteThread'.
 *
 ...
 *                | Server error                | 500               |
 */
export const threadRouterDeleteThread = Router(options).delete(
  '/',
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'threadValidationSchemaDeleteThread'.
      ...
      const stat = await threadController.deleteThread(validatedParams);
